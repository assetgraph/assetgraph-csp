...
reviewContentSecurityPolicy.installInto = AssetGraph => {
  // FIXME: Ideally these would only be registered once,
  // and it would have a nicer syntax
  AssetGraph.registerAsset(require('./ContentSecurityPolicy'));
  AssetGraph.Html.registerRelation(require('./HtmlContentSecurityPolicy'));
};

/* eslint-disable no-inner-declarations */
function reviewContentSecurityPolicy(
  assetGraph,
  { query, includePath = false, update = false, level } = {}
) {
  reviewContentSecurityPolicy.installInto(assetGraph.constructor);
  ...
  for (const htmlAsset of assetGraph.findAssets(
    ...
    for (const htmlContentSecurityPolicy of htmlContentSecurityPolicies) {
      ...
      function noteAsset(asset, incomingRelation, directive) {
        directive =
          directive ||
          (incomingRelation && directiveByRelationType[incomingRelation.type]);
        if (directive) {
          if (asset.isInline) {
            if (
          ...
          if (
            incomingRelation.from.type === 'Html' &&
            incomingRelation.node // Avoid HttpRedirect etc.
          ) {
      ...
      for (const directive of directives) {
        const originsToAdd = Object.keys(
          disallowedRelationsByDirectiveAndOrigin[directive] || {}
        );
        let existingOrigins;
              ...
              if (relations.length > 0) {
                if (nonWhitelistedOrigin === "'unsafe-hashes'") {
                  if (level >= 3) {
                    assetGraph.warn(
                      new Error(
                        `${htmlAsset.urlOrDescription}: Missing ${fromCamelCase(
                          directive
                        )} 'unsafe-hashes':\n  ${relations
...
module.exports = reviewContentSecurityPolicy;
