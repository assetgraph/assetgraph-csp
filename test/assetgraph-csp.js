...
const assetGraphCsp = require('../lib/assetgraph-csp');
...
describe('transforms/reviewContentSecurityPolicy', function() {
  it('should not do anything for Html assets that do not have an existing policy', async function() {
    const assetGraph = new AssetGraph({
      root: pathModule.resolve(
        __dirname,
        '../testdata/noExistingContentSecurityPolicy/'
      )
    });
    await assetGraph.loadAssets('index.html');
    await assetGraph.populate();
    await assetGraphCsp(assetGraph, { update: true });
  ...
  it('should preserve existing tokens, even if they appear superfluous', async function() {
    const assetGraph = new AssetGraph({
      root: pathModule.resolve(
        __dirname,
        '../testdata/existingContentSecurityPolicy/extraDirectives/'
      )
    });
    await assetGraph.loadAssets('index.html');
    await assetGraph.populate();
  ...
  it('should copy all tokens from default-src when introducing a new directive', async function() {
    const assetGraph = new AssetGraph({
      root: pathModule.resolve(
        __dirname,
        '../testdata/existingContentSecurityPolicy/defaultSrcWithExtraSourceExpressions/'
      )
    });
    await assetGraph.loadAssets('index.html');
    await assetGraph.populate();
  ...
  describe('when assets are present on other domains', function() {
    describe('with includePath enabled for script-src and style-src', function() {
      it('should update the style-src and script-src directives of a Content-Security-Policy when no existing source expression allows the url', async function() {
        const assetGraph = new AssetGraph({
          root: pathModule.resolve(
            __dirname,
            '../testdata/existingContentSecurityPolicy/externalScriptAndStylesheet'
          )
        });
        await assetGraph.loadAssets('index.html');
        await assetGraph.populate();
        ...
        await assetGraphCsp(assetGraph, {
          update: true,
    ...
    it('should just whitelist the host:port of the origin for less sensitive media types such as images', async function() {
      const assetGraph = new AssetGraph({
        root: pathModule.resolve(
          __dirname,
          '../testdata/existingContentSecurityPolicy/image/'
        )
      });
      await assetGraph.loadAssets('index.html');
      await assetGraph.populate();
  ...
  it('should update the image-src accordingly when images are included as data: urls', async function() {
    const assetGraph = new AssetGraph({
      root: pathModule.resolve(
        __dirname,
        '../testdata/existingContentSecurityPolicy/smallImage/'
      )
    });
    await assetGraph.loadAssets('index.html');
    await assetGraph.populate();
  ...
  it("should not leave 'none' in the list of allowed origins when adding to an existing policy", async function() {
    const assetGraph = new AssetGraph({
      root: pathModule.resolve(
        __dirname,
        '../testdata/existingContentSecurityPolicy/scriptSrcNone/'
      )
    });
    await assetGraph.loadAssets('index.html');
    await assetGraph.populate();
  ...
  it("should include hash-source fragments and add 'unsafe-inline' for inline scripts and stylesheets when 'unsafe-inline' is not in the existing policy", async function() {
    const assetGraph = new AssetGraph({
      root: pathModule.resolve(
        __dirname,
        '../testdata/existingContentSecurityPolicy/inlineScriptAndStylesheet/'
      )
    });
    await assetGraph.loadAssets('index.html');
    await assetGraph.populate();
  ...
  it("should upgrade nonces to hashes and remove the nonce attributes (and add 'unsafe-inline' for CSP1 compatibility)", async function() {
    const assetGraph = new AssetGraph({
      root: pathModule.resolve(
        __dirname,
        '../testdata/existingContentSecurityPolicy/inlineScriptAndStylesheetWithNonces/'
      )
    });
    await assetGraph.loadAssets('index.html');
    await assetGraph.populate();
  ...
  it('should remove a nonce from the CSP when relations with the same nonce have been bundled (integration with bundleRelations)', async function() {
    const assetGraph = new AssetGraph({
      root: pathModule.resolve(
        __dirname,
        '../testdata/existingContentSecurityPolicy/bundledScriptsWithSameNonce/'
      )
    });
    await assetGraph.loadAssets('index.html');
    await assetGraph.populate();
  ...
  it('should always remove a nonce with a value of "developmentonly", even when it is not referenced by a <script> or <link>', async function() {
    const assetGraph = new AssetGraph({
      root: pathModule.resolve(
        __dirname,
        '../testdata/existingContentSecurityPolicy/unreferencedDevelopmentOnlyNonce/'
      )
    });
    await assetGraph.loadAssets('index.html');
    await assetGraph.populate();
  ...
  it("should leave the empty script as an allowed hash when removing a nonce so 'unsafe-inline' would be left alone and thus take effect with CSP2+", async function() {
    const assetGraph = new AssetGraph({
      root: pathModule.resolve(
        __dirname,
        '../testdata/existingContentSecurityPolicy/upgradedNonceAndUnsafeInline/'
      )
    });
    await assetGraph.loadAssets('index.html');
    await assetGraph.populate();
  ...
  it("should leave the empty script as an allowed hash when removing nonces so 'unsafe-inline' would be left alone and thus take effect with CSP2+", async function() {
    const assetGraph = new AssetGraph({
      root: pathModule.resolve(
        __dirname,
        '../testdata/existingContentSecurityPolicy/nonceDevelopmentOnlyAndUnsafeInline/'
      )
    });
    await assetGraph.loadAssets('index.html');
    await assetGraph.populate();
  ...
  describe('with update:false', function() {
    it('emits a warn event when an inline relation is prohibited by the policy', async function() {
      ...
      expect(warnSpy, 'to have calls satisfying', async function() {
        warnSpy(
          new Error(
            "testdata/existingContentSecurityPolicy/inlineScriptAndStylesheet/index.html: An asset violates the style-src 'self' Content-Security-Policy directive:\n" +
              '  inline Css in testdata/existingContentSecurityPolicy/inlineScriptAndStylesheet/index.html'
          )
        );
        warnSpy(
          new Error(
            "testdata/existingContentSecurityPolicy/inlineScriptAndStylesheet/index.html: An asset violates the script-src 'self' Content-Security-Policy directive:\n" +
              '  inline JavaScript in testdata/existingContentSecurityPolicy/inlineScriptAndStylesheet/index.html'
          )
        );
      });
    });
    ...
    it('should emit a warn even when a relation to an asset on a non-whitelisted origin is present', async function() {
      ...
      expect(warnSpy, 'to have calls satisfying', async function() {
        warnSpy(
          new Error(
            'testdata/existingContentSecurityPolicy/inlineScriptAndStylesheet/index.html: An asset violates the script-src foo.com Content-Security-Policy directive:\n' +
              '  http://bar.com/yadda.js'
          )
        );
      });
    ...
    it("emits a warn event when a relation is prohibited by a 'none' directive", async function() {
      ...
      expect(warnSpy, 'to have calls satisfying', async function() {
        warnSpy(
          new Error(
            "testdata/existingContentSecurityPolicy/scriptSrcNone/index.html: An asset violates the script-src 'none' Content-Security-Policy directive:\n" +
              '  testdata/existingContentSecurityPolicy/scriptSrcNone/script.js'
          )
        );
      });
    });
  ...
  it("should hash an inline script when there is a nonce, even when 'unsafe-inline' is permitted", async function() {
    const assetGraph = new AssetGraph({
      root: pathModule.resolve(
        __dirname,
        '../testdata/existingContentSecurityPolicy/nonceAndUnsafeInline/'
      )
    });
    await assetGraph.loadAssets('index.html');
    await assetGraph.populate();
  ...
  it('should return an info object', async function() {
    ...
    const infoObject = await assetGraphCsp(assetGraph, {
      update: true,
      level: 1
    });
      ...
      describe('with level=3', function() {
        it("should add a hash of attribute and 'unsafe-hashes' to style-src", async function() {
          const assetGraph = new AssetGraph({
            root: pathModule.resolve(
              __dirname,
              '../testdata/existingContentSecurityPolicy/inlineStyleAttribute/'
            )
          });
          await assetGraph.loadAssets('index.html');
          await assetGraphCsp(assetGraph, {
    ...
    describe('in validation mode', function() {
      describe('with level=3', function() {
        it("should emit two warnings, one about 'unsafe-hashes' missing and one about the hash source missing", async function() {
          ...
          assetGraph.on('warn', warnSpy);
          await assetGraph.loadAssets('index.html');
          await assetGraphCsp(assetGraph, { level: 3 });
          ...
          expect(warnSpy, 'to have calls satisfying', () => {
            ...
            warnSpy(
              "testdata/existingContentSecurityPolicy/inlineStyleAttribute/index.html: Missing style-src 'unsafe-hashes':\n" +
                '  inline Css in testdata/existingContentSecurityPolicy/inlineStyleAttribute/index.html'
            );
          });
        });
      });
      ...
      describe('with level=2', function() {
        it("should add 'unsafe-inline' instead of 'unsafe-hashes' and emit a warning", async function() {
          const assetGraph = new AssetGraph({
            root: pathModule.resolve(
              __dirname,
              '../testdata/existingContentSecurityPolicy/inlineEventHandler/'
            )
          });
          const warnSpy = sinon.spy().named('warn');
          assetGraph.on('warn', warnSpy);
          ...
          expect(warnSpy, 'to have calls satisfying', () => {
            warnSpy(
              `testdata/existingContentSecurityPolicy/inlineEventHandler/index.html contains one or more inline event handlers, which cannot be whitelisted with CSP level 2 except via 'unsafe-inline', which almost defeats the purpose of having a CSP\n` +
        ...
        describe("when the existing CSP allows 'unsafe-inline'", function() {
          it("should not add 'unsafe-hashes' or any hashes", async function() {
            const assetGraph = new AssetGraph({
              root: pathModule.resolve(
                __dirname,
                '../testdata/existingContentSecurityPolicy/inlineEventHandlerWithUnsafeInline/'
              )
            });
            await assetGraph.loadAssets('index.html');
            await assetGraphCsp(assetGraph, {
          ...
          it("should not add the hash for a script as that would shadow 'unsafe-inline' and thus prohibit the event handler", async function() {
            const assetGraph = new AssetGraph({
              root: pathModule.resolve(
                __dirname,
                '../testdata/existingContentSecurityPolicy/unsafeInlineWithInlineScriptAndEventHandler/'
              )
            });
            await assetGraph.loadAssets('index.html');
            await assetGraphCsp(assetGraph, {
    ...
    describe('in validation mode', function() {
      describe('with level=2', function() {
        it("should emit a warning about 'unsafe-inline' missing", async function() {
          ...
          assetGraph.on('warn', warnSpy);
          await assetGraph.loadAssets('index.html');
          await assetGraphCsp(assetGraph, { level: 2 });
      ...
      describe('with level=3', function() {
        it("should emit two warnings, one about 'unsafe-hashes' missing and one about the hash source missing", async function() {
          ...
          expect(warnSpy, 'to have calls satisfying', () => {
            ...
            warnSpy(
              "testdata/existingContentSecurityPolicy/inlineEventHandler/index.html: Missing script-src 'unsafe-hashes':\n" +
                '  inline JavaScript in testdata/existingContentSecurityPolicy/inlineEventHandler/index.html'
            );
          });
        });
