...
expect.addAssertion('<string> [not] to allow <string> <string?>', function(
  expect,
  sourceExpression,
  url,
  ...
  expect(
    matchSourceExpression(
      url,
      sourceExpression,
      protectedResourceUrl || 'http://example.com/'
    ),
    '[not] to be true'
  );
});
  ...
  describe("'self'", function() {
    ...
    it('should allow a file: url when the protected resource has a file: url', function() {
      expect(
        "'self'",
        'to allow',
        'file:///home/andreas/work/webmail/http-pub/common/requireJsConfig.js',
        'file:///home/andreas/work/webmail/http-pub/unsupported.html'
      );
    });
  });
});
